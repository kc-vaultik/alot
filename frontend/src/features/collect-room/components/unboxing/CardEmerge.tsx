...
import { getGradientColors, getBorderGlow, getGlowColor, getAmbientBackground } from '@/utils/styling';
import alotLogo from '@/assets/alot-logo.png';

interface CardEmergeProps {
  card: CollectCard;
  onReveal: () => void;
}

export const CardEmerge = memo(({ card, onReveal }: CardEmergeProps) => {
  ...
  // Enhanced parallax with more depth
  ...
  // Additional depth transforms for holographic effect
  const glowIntensity = useTransform(mouseX, [-200, 200], [0.3, 0.8]);

  useEffect(() => {
    // Slower slide timing for more drama
  ...
  // Get styling from shared utilities
  const gradientColors = getGradientColors(tier);
  const borderGlow = getBorderGlow(tier);
  const glowColor = getGlowColor(tier);
  const ambientBg = getAmbientBackground(tier);
  ...
  return (
    ...
    >
      {/* Enhanced ambient background glow based on rarity */}
      ...
      {/* Card container - slower, more dramatic slide in */}
      <motion.div
        ref={cardRef}
        ...
        onClick={hasSlid ? onReveal : undefined}
      ...
      >
        ...
        {/* Main card - Collect Card Template Style (Face down) with 3:4.5 aspect ratio */}
        <div className="relative w-72 sm:w-80 aspect-[3/4.5]">
          <div className="absolute inset-0 bg-[#0a0a0f] rounded-2xl overflow-hidden border-4 border-white shadow-sticker">
            ...
            {/* Card Header - matching CardPreview.tsx positioning */}
            ...
            {/* Enhanced holographic shine effect - multiple layers */}
            <motion.div
            ...
            {/* Center mystery icon with enhanced glow */}
            ...
            {/* Footer text placeholders - matching CardPreview.tsx structure */}
      ...
      >
        ...
        >
          Your card is emerging from the pack...
        </motion.p>
        <p className="text-white/70 text-lg tracking-wide mb-2">
          Tap to reveal your shards
        </p>
        <p className="text-white/30 text-xs">
          See which product you're progressing toward and how many shards you've earned
        </p>
      </motion.div>
    </motion.div>
  );
...
CardEmerge.displayName = 'CardEmerge';
